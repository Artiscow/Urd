...
```json
{
  ...
  "background": {
    "version": 1,
    "layers": [
      { "type": "gradient", "version": 3, "props": { "kind": "linear", "stops": [{ "color": "#0b0e14", "share": 50 }, { "color": "#1a1030", "share": 50 }], "angle": 160, "x": 0.5, "y": 0.5, "animation": "none" } },
...
- **`boxStyle`** (valgfri, additiv fra v0.6, på tekstboksen og faq-kortene): `{ "shadow": "soft" | "strong", "border": "none" | { "color": "#…" | "<token>", "width": 1-4 }, "glass": true }`. Alle felt valgfrie; utelatt felt = basisstilen i `.urd-text-box` (temaets flatefarge og tynne kantlinje). `glass` gir frostet glass (gjennomskinnelig flate + `backdrop-filter: blur`); uten nettleserstøtte står den gjennomskinnelige flaten igjen. Ren logikk i `engine/box-style.js` (`boxStyleCss`).
