...
- **`image`-laget er versjon 2 fra v0.6**: `parallax` (0..1, additivt, standard 0) lar bildet henge etter når man ruller (`transform: translateY` styrt av seksjonens plass i viewporten, ren `parallaxOffset`; AV på mobil og ved `prefers-reduced-motion`). Migrering v1 → v2 setter `parallax: 0`. Én passiv scroll-/resize-lytter på modulnivå driver alle parallax-lag via `requestAnimationFrame` (frakoblede lag lukes ut).
