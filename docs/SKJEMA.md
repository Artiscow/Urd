...
- **`theme`** (valgfri streng, additiv fra v0.6): ferdig seksjonstema (rollesett) som overstyrer temaets fargetokens KUN på denne seksjonen. Verdier: `surface` (`bg` = surface), `accent` (`bg` = accent, `text` = accent-text, accent↔accent-text byttes så knapp/lenke inverterer), `inverse` (`bg` = text, `text` = bg), og fra 0.6.6.4.6 også `soft` (aksenten som svakt pastell-skjær i bg/surface), `muted` (gråtonet sone med mykere tekst), `deep` (invers med aksentskjær i bg/surface) og `highlighted` (kun surface tones i aksenten). Rolleverdiene var norske (flate/aksent/invers/dus/dempet/dyp/uthevet) til schemaVersion 2; sidemigreringen 2 → 3 løfter dem (ADR-0021). Fravær = Standard (ingen overstyring). Overstyringene settes som `--urd-color-*` på seksjonens element og refererer BASIS-kopiene `--urd-base-*` (satt av `applyTheme`), aldri de levende `--urd-color-*` - ellers ville invers lage en `var()`-sykel. Fordi de er referanser, følger rollene lys/mørk automatisk. Blokkene arver via `resolveColor` → `var(--urd-color-*)`. Ukjent rolle ignoreres. Definert i `SECTION_THEMES` (`engine/theme.js`).
...
- **`background.layers`** rendres i rekkefølge. Kjernelagtyper: `color`, `gradient` (også animert), `glow`, `image`, `grain`, `slideshow` (fra v0.6) og `video` (fra v0.7). Plugins kan definere flere.
- **`image`-laget**: et indre `.urd-bg-image`-element bærer bildet, og posisjon/størrelse styres CSS-native (samme modell som slideshow-laget), IKKE via transform. Modellen er FRI PLASSERING (som Figma Crop / Webflow Custom): en skala og en posisjon som virker for alle bilder. **`fit`**: `plain` (fri plassering, standard) eller `tile` (gjentar bildet som mønster); `custom` setter egen skala. De gamle norske verdiene (`vanlig`, `flislegg`, `egen`) løftes av lagets 1 -> 2-migrering (ADR-0021). `cover`/`contain` beholdes som nøkkelord for bakoverkompat og for slideshow-laget. **`size`** (brøk, standard 1 = 100 %, klemt til 0.1-4 i editoren, ren `bgSize`) er breddrelativ SKALA: `background-size: {size*100}%` (auto høyde beholder forholdet), så 100 % = like bred som seksjonen. Editorens **Dekk**/**Vis hele**-knapper regner ut skalaen som fyller/viser hele bildet fra bilde- og seksjonsmål. **`x`/`y`** (standard 0.5 = sentrert) er plassering via prosent-`background-position` (ren `bgPosition`) og kan gå UNDER 0 / OVER 1 (editoren: -0.5..1.5) for å legge motivet delvis eller helt utenfor kanten. **Opplastede SVG-er auto-trimmes** (editoren måler motivets omfang via canvas-piksler og strammer `viewBox`, ren `tightSvgViewBox`/`svgViewBox`), så død plass rundt en logo ikke forstyrrer skala/posisjon. **`blur`** (px) gjør bildet uskarpt uten forsettlig forstørring (elementet strekkes kun så vidt utover kanten at blur-randen klippes bort). **`parallax`** (0..1, additivt, standard 0) lar bildet henge etter når man ruller: styrken styrer FARTEN (`parallaxOffset`, `MAX_SHIFT` = 0.4 for kraftig utslag). Den frie modellen (`plain`, som viser bildet med luft rundt) forskyves rent med `translateY` UTEN overskann, så parallaksen er zoom-fri og kraftig (større tak i `parallaxPad`). Fyllmodus (`cover`/`tile`) MÅ overskanne (gap ved kanten er uakseptabelt; for `cover` gir det en liten zoom, strammere tak). AV på mobil og ved `prefers-reduced-motion`. **`bleed`** (`none`/`up`/`down`/`both`, additiv, standard `none`) lar parallaksen flyte forbi seksjonskanten inn i naboen via retnings-`clip-path` (ren `bleedClip`; sidene alltid klippet så ingen sidescroll). `down`/`both` løfter laget til `z-index: 1` fordi neste seksjon kommer senere i DOM og ellers maler bakgrunnen sin oppå bleeden; laget maler da over naboens BAKGRUNN, men fortsatt under innholdet dens (blokker ligger på z>=1 senere i treet). `up` maler over forrige seksjon via tre-rekkefølgen. Laget klippes ellers til seksjonsboksen. Én passiv scroll-/resize-lytter på modulnivå driver alle parallax-lag via `requestAnimationFrame` (frakoblede lag lukes ut).
...
- **`slideshow`-laget** (hero-galleri) blar gjennom flere bilder med myk krysstoning: `{ "images": [{ "src": "/media/…", "x": 0.5, "y": 0.5 }], "fit": "cover", "interval": 6, "fade": 1.5, "opacity": 1, "blur": 0 }`. `x`/`y` (0..1) er fokuspunkt per bilde, `interval` er sekunder mellom bytter, `fade` er toningens lengde i sekunder. Med ett bilde, eller redusert bevegelse hos den besøkende, vises kun det første bildet statisk.
- **`video`-laget** (fra v0.7, funksjonskartet C6): `{ "src": "/media/….mp4", "poster": "/media/….webp", "fit": "cover", "x": 0.5, "y": 0.5, "opacity": 1, "parallax": 0 }`. Selvhostet mp4/webm-loop, git-eid som bildene (data-URL i utkast, materialiseres til `media/` ved publisering); `poster` er valgfritt stillbilde. Avspilling: dempet loop (`muted`/`playsinline`) som STARTES av delt IntersectionObserver og pauses utenfor viewporten; `preload="metadata"` gjør at filmen først hentes når seksjonen nærmer seg. `fit`/`x`/`y` styrer utsnittet via `object-fit`/`object-position`. `parallax` (0..1) bruker bildelagets delte maskineri (scroll-drevet CSS med rAF-fallback). Ved `prefers-reduced-motion` spilles aldri video: plakaten vises som stillbilde, og uten plakat utelates laget så lagene under synes. Kilden vokter-valideres (ankret regex: kun `/media/*.mp4|webm` og video-data-URL-er).
- **Mediegrensene** (satt samlet, 0.7.7, konstantene i `engine/imageTools.js`): bilder komprimeres i nettleseren til webp (maks 1600 px lengste side) med varsel over 400 kB (`WARN_BYTES`); lyd publiseres uendret med samme varselgrense; video varsles over 4 MB (`VIDEO_WARN_BYTES`, base64-utkastet kan sprenge localStorage-kvoten, da består utkastet kun i minnet til publisering) og avvises hardt over 15 MB (`VIDEO_MAX_BYTES`, godt under Cloudflare Pages' filgrense på 25 MiB). Publiserings-vokterens medieendelser speiler medietypene (bilde + lyd + mp4/webm).
...
- **`type`** slår opp i blokkregisteret (`Urd.blocks`). Kjerneblokker: `text`, `image`, `button`, `shape` (streker - horisontale, vertikale og skrå via `rot` - sirkler, rektangler), `video` (YouTube/Vimeo med personvennlig innbygging; CSP-en har et bevisst frame-src-unntak for de to vertene), `icon` (glyf/emoji med størrelse og temafarge), `collection` (se under), `gallery` (fra v0.6, se under), `faq` (fra v0.6, se under), `timeline`, `quote` og `stats` (fra v0.6, se under), samt `table`, `share`, `countdown`, `audio`, `product`, `cart` og `checkout` (fra v0.7, se under). Plugins kan definere flere.
- **`faq`-blokkens props** (fra v0.6): `{ "items": [{ "q": "Spørsmål?", "a": "<p>Svar (rik tekst)</p>" }], "multi": false, "boxStyle": null }`. Akkordeon etter disclosure-mønsteret (knapp med `aria-expanded`, aldri `role="menu"`): svaret foldes ut ved klikk, `multi` lar flere svar stå åpne samtidig. Blokkens lagrede høyde er alltid den sammenfoldede (autovekst som collection-blokken, kun høyden meldes); utfolding vokser kun visuelt. `boxStyle` er kortstilen (se `boxStyle` under).
- **`timeline`-blokkens props** (fra v0.6): `{ "items": [{ "year": "2019", "title": "…", "text": "…" }], "variant": "left" | "alternating", "marker": "filled" | "ring", "accent": null }`. Vertikal hendelsesliste langs en CSS-tegnet linje; `alternating` legger kortene annenhver side av en midtlinje. `accent` er tematoken eller hex (null = temaets aksent). Autovekst som faq (kun høyden meldes). Alle tekstfeltene er ren tekst.
- **`quote`-blokkens props** (fra v0.6): `{ "text": "…", "attribution": "…", "role": "…", "variant": "large" | "short", "image": "", "accent": null }`. Semantisk `figure`/`blockquote` med attribusjon i `figcaption`; `short` er testimonial-kortet med valgfritt portrett (`image` er sti i `media/`, data-URL i utkast). Anførselsglyfen tegnes i CSS. `accent` som tidslinjen. Autovekst (kun høyden meldes).
- **`stats`-blokkens props** (fra v0.6): `{ "value": "4800", "prefix": "", "suffix": "+", "label": "…", "countUp": true }`. Ett nøkkeltall med etikett; `countUp` teller opp fra null ved første entré hos besøkende (engangs-IntersectionObserver) når `value` er et rent tall (mellomrom og desimal tillatt) - ellers, ved redusert bevegelse og i editoren står tallet i sluttilstand (ADR-0011). Autovekst (kun høyden meldes).
- **`table`-blokkens props** (fra v0.7): `{ "header": true, "striped": false, "lines": "rows" | "grid" | "none", "rows": [["Dag", "Åpent", "Merknad"], ["Mandag", "10-16", ""]] }`. Semantisk `<table>` i en overflow-x-wrapper (brede tabeller ruller i egen flate). `rows` er radene som rene tekststrenger; første rad tegnes som overskriftsrad når `header` er sann. Cellene redigeres på lerretet, rader/kolonner i Egenskaper. Autovekst (kun høyden meldes).
- **`share`-blokkens props** (fra v0.7): `{ "services": ["facebook", "x", "linkedin", "whatsapp", "email", "copy"], "variant": "icons" | "labels", "size": 38, "color": "" }`. Statiske delingslenker uten sporing (aldri leverandør-SDK): adressen som deles leses ved klikk. `copy` bruker Clipboard API og vises kun der API-et finnes. `color` er tematoken eller hex (tom = aksent). Ikonene er ikonbibliotekets tegnede SVG-er.
- **`countdown`-blokkens props** (fra v0.7): `{ "target": "2026-12-24T18:00", "doneText": "…", "variant": "boxes" | "plain", "showSeconds": true }`. Teller ned mot `target` (datetime-local-form, lokal tid); passert mål viser `doneText`. Enhetsordene er egne oversettelsesnøkler i stedet for `Intl.RelativeTimeFormat` (nordsamisk mangler i ICU). Tikkingen er setInterval-logikk, ikke animasjon (ADR-0011).
- **`audio`-blokkens props** (fra v0.7): `{ "src": "/media/…", "title": "", "loop": false }`. Native `<audio controls preload="metadata">` med git-eid fil (data-URL i utkast, skrives til `media/` ved publisering som bildene); CSP-ens `default-src 'self'` dekker avspillingen. `title` vises over spilleren og kan skrives på lerretet.
- **`product`-blokkens props** (fra v0.7, butikken): `{ "collection": "produkter", "limit": 0, "columns": 0, "currency": "kr" }`. Rendrer innslagene i en produktsamling (`kind: "products"`, se samlinger under) som kort med bilde, badge, pris/medlemspris, størrelses- og fargevalg og «Legg i handlekurv». Et fargevalg med eget bilde bytter kortets bilde. `limit`/`columns` 0 = alle/automatisk; `currency` er ordet etter prisen. Kjøpsknappen skriver til kurven (`engine/shop.js`, localStorage-nøkkelen `urd-cart`, hendelsen `urd-cart-change`). Hos besøkende åpner klikk på bilde/tittel en quick view: produktdetaljene i en native `<dialog>` (ADR-0011) med slideshow (hovedbilde + fargebilder), full tekst, variantvalg og kjøpsknapp; prisraden vises kun når `price` er satt. I editoren eier klikk redigeringen, og et «+ Produkt»-kort sist i rutenettet melder `urd-collection-add {collection}` til editoren, som legger et nytt produkt i samlingen. Autovekst (kun høyden meldes, som collection). Blokkens `animation`/`hover`-felt spilles kortvis (def-flagget `animPerCard`): inngangen spilles per kort med forskjøvet start fra blokkens synlighet, og pekereffekten løfter kort for kort.
- **`cart`-blokkens props** (fra v0.7, butikken): `{ "variant": "button" | "icon", "href": "", "currency": "kr" }`. Knapp med antall-badge som åpner en kurvskuff bygget på native `<dialog>`/showModal (ADR-0011); skuffen viser linjene med antall-styring, sum og, når `href` er satt, en «Til kassen»-lenke til kassesiden. Kurven bor hos den besøkende (localStorage): ingen konto, ingen sporing, ingen nettverkskall.
- **`checkout`-blokkens props** (fra v0.7, butikken): `{ "recipient": "", "endpoint": "", "vipps": "", "currency": "kr", "vippsCheckout": false }`. `vippsCheckout` (additivt, [ADR-0020](adr/0020-valgfritt-betalingslag-vipps-checkout.md)) viser en «Betal med Vipps»-knapp: kurven POST-es til sidens egen funksjon `/api/vipps/checkout`, som regner summen på nytt fra den git-eide katalogen (aldri klientens tall) og redirecter til Vipps Checkout; retur til kassesiden med `?ordered=1` gir kvittering og tømmer kurven. Krever Cloudflare-hemmelighetene `VIPPS_CLIENT_ID`/`VIPPS_CLIENT_SECRET`/`VIPPS_SUBSCRIPTION_KEY`/`VIPPS_MSN`; uten dem degraderer knappen pent, og skjema-kassen forblir standard. Bestillingsskjema uten betalingsgateway: ordresammendraget leses fra kurven, kontaktfeltene (navn, e-post, telefon, kommentar) valideres, og bestillingen sendes som e-postutkast til `recipient` (mailto, null oppsett) eller som JSON til `endpoint` (POST; ekstern vert krever `connect-src` i `_headers`, ADR-0006 - samme flyt som skjema-pluginen og footer-nyhetsbrevet). Endepunktet vinner når begge er satt; bekreftet endepunkt-svar tømmer kurven, mens mailto beholder den (utkastet kan avbrytes). Honeypot-felt mot bots (utfylt = forkastet i stillhet). `vipps` vises som betalingsinstruks under skjemaet. Ren ordre-logikk i `engine/shop.js` (`orderLines`, `buildOrderBody`, `buildOrderMailto`, `buildOrderPayload`).
...
- **`gallery`-blokkens props**: `{ "images": [{ "src": "/media/…", "alt": "…", "href": null, "style": { "fit": "cover", "x": 0.5, "y": 0.5, "zoom": 1, "brightness": 1, "contrast": 1, "saturate": 1 } }], "view": "grid", "columns": 3, "gap": 12, "radius": "md", "lightbox": true, "interval": 5 }`. `view` er `grid` (rutenett), `carousel` (sidescroll) eller `slides` (lysbilde med automatisk fremrykk hvert `interval` sekund; står stille ved redusert bevegelse). `style` per bilde er samme ikke-destruktive vokabular som bildeblokken. `lightbox` åpner bildet i fullskjerm ved klikk hos besøkende; `href` per bilde vinner over lightboxen. Rutenettet vokser automatisk med innholdet (kun høyden meldes, som collection-blokken).
...
collection-blokken med valgbar visning (`cards`/`list`/`archive`). Kontrakten
...
- **`kind`** (`news`/`notices`/`publications`/`products`/`custom`) styrer hvilke felter editoren fremhever; visningene leser de felles feltnavnene.
- **Produktfeltene** (kind `products`, additive fra v0.7, leses av produkt-blokken): `price` (tall, valuta-ordet settes på blokken), `memberPrice` (valgfri rabattert pris), `badge` (kort merkelapp på kortet), `sizes` (liste av størrelsesnavn, vises som valg-chips) og `colors` (`[{name, image?}]`; et fargevalg med eget bilde bytter kortets bilde). Katalogen er git-eid som andre samlinger (ADR-0007).
...
- **CSV-import/-eksport** (fra v0.7, funksjonskartet C12): Samlinger-panelet eksporterer innslagene som CSV og importerer en CSV-fil som ERSTATTER innslagene (første rad er kolonnenavnene; listefeltene `sizes`/`colors` skilles med `|`, fargebilder følger ikke med). Ren logikk i `engine/collections-csv.js`, som kun editoren bundler - besøkende laster den aldri.
...
- **Filnivå:** `schemaVersion` løftes med samme stegvise mønster for strukturelle endringer, implementert i `liftPageFile()`/`liftSiteFile()` i migrate.js. Sidetabellen har stegene 1 → 2 (mobilmodellen, ADR-0019), 2 → 3 (kontrakt-tokens til engelsk, ADR-0021) og 3 → 4 (kjerne-preset-idene, ADR-0021); mal-nyttelaster løftes tilsvarende ved innsetting via `liftContractTokens` (templates-model.js), siden de settes inn utenom sideløftet.
