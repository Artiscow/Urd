...
```json
{
  ...
  "layout": { "contentWidth": 1200, "gutter": 24 },
...
- **`layout`** (valgfri, additiv fra v0.7, ADR-0018) binder innholdet til en designbredde: seksjonen er full vindusbredde og eier bakgrunnen, mens blokkene ligger i en sentrert innholdsflate (`.urd-canvas`) på `min(100% - 2*gutter, contentWidth)`. **`contentWidth`** er tall i px (standard 1200) eller `"full"` for ubunden fullbredde (Urds oppførsel før v0.7); tall og ikke CSS-lengde, siden editoren regner forhåndsvisningens målbredde ut fra den. **`gutter`** (standard 24) er minste sidemarg mot vinduskanten. Bindingen gjelder KUN over `breakpoints.mobile`: under brekkpunktet er margen 0 og flaten 100 %, så lagrede `frames.mobile` beholder betydningen sin. Site-migreringen 1 → 2 fyller feltet inn eksplisitt, så motoren og editoren aldri kan utlede hver sin standard. Per seksjon overstyres bredden med `size.maxWidth`.
...
- **`sticky`** (valgfri, additiv fra v0.6): «fest ved scrolling» - `{ "offset": 16, "until": null }`. Blokken festes `offset` px fra vindustoppen når den scrolles dit, og slipper når egen seksjon er forbi (`until: null`) eller først når seksjonen med id `until` er passert. Er en topplinje klistret, legges menyhøyden automatisk til avstanden, så blokken fester seg under menyen i stedet for bak den. Kun desktop; mobil-flyten ignorerer feltet. Festingen er JS-styrt (`engine/sticky.js` + rene funksjoner i `sticky-model.js`); i editorens preview er den aktiv også med redigeringshåndtak på, men suspenderes mens en blokk dras eller endrer størrelse. En ukjent/slettet `until`-id degraderer til egen seksjons grense. Tre additive felt fra v0.7:
  ...
  - **`dock`** (standard `bottom-right`): ankerpunktet ved `mode: 'screen'`, satt sammen av vertikal og horisontal akse (`top`/`middle`/`bottom` og `left`/`center`/`right`). `offset` er da avstanden fra de kantene blokken dokkes til; senterakser sentreres og ignorerer den.
