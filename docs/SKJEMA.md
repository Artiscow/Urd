...
- **`background.layers`** rendres i rekkefølge. Kjernelagtyper: `color`, `gradient` (også animert), `glow`, `image`, `grain`, `bildegalleri` (fra v0.6) og `video` (fra v0.7). Plugins kan definere flere.
...
- **`type`** slår opp i blokkregisteret (`Urd.blocks`). Kjerneblokker: `text`, `image`, `button`, `shape` (streker - horisontale, vertikale og skrå via `rot` - sirkler, rektangler), `video` (YouTube/Vimeo med personvennlig innbygging; CSP-en har et bevisst frame-src-unntak for de to vertene), `icon` (glyf/emoji med størrelse og temafarge), `samling` (se under), `galleri` (fra v0.6, se under), `faq` (fra v0.6, se under), `tidslinje`, `sitat` og `statistikk` (fra v0.6, se under), samt `tabell`, `deling`, `nedteller`, `audio`, `produkt`, `handlekurv` og `kasse` (fra v0.7, se under). Plugins kan definere flere.
...
- **`tabell`-blokkens props** (fra v0.7): `{ "header": true, "striped": false, "lines": "rows" | "grid" | "none", "rows": [["Dag", "Åpent", "Merknad"], ["Mandag", "10-16", ""]] }`. Semantisk `<table>` i en overflow-x-wrapper (brede tabeller ruller i egen flate). `rows` er radene som rene tekststrenger; første rad tegnes som overskriftsrad når `header` er sann. Cellene redigeres på lerretet, rader/kolonner i Egenskaper. Autovekst (kun høyden meldes).
- **`deling`-blokkens props** (fra v0.7): `{ "services": ["facebook", "x", "linkedin", "whatsapp", "email", "copy"], "variant": "icons" | "labels", "size": 38, "color": "" }`. Statiske delingslenker uten sporing (aldri leverandør-SDK): adressen som deles leses ved klikk. `copy` bruker Clipboard API og vises kun der API-et finnes. `color` er tematoken eller hex (tom = aksent). Ikonene er ikonbibliotekets tegnede SVG-er.
- **`nedteller`-blokkens props** (fra v0.7): `{ "target": "2026-12-24T18:00", "doneText": "…", "variant": "boxes" | "plain", "showSeconds": true }`. Teller ned mot `target` (datetime-local-form, lokal tid); passert mål viser `doneText`. Enhetsordene er egne oversettelsesnøkler i stedet for `Intl.RelativeTimeFormat` (nordsamisk mangler i ICU). Tikkingen er setInterval-logikk, ikke animasjon (ADR-0011).
