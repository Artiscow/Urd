...
**Planlagt (B):** multimarkering med kopier/lim og align/distribute, hjelpelinjer, sticky blokker, «+ ny blokk» der man klikker, seksjonshøyde ovenfra, boks-/kortstiler, seksjonstemaer, gradient-editor (0.6.6, siden levert i 0.6.6.3/0.6.6.4); lagpanel (0.7.9, flyttet dit 9. august 2026); mobil-revurdering (0.7.3, levert 13. august 2026 i ADR-0019). Nettbrett-breakpoint er bevisst utsatt (VEIKART) - Webflows sju-breakpoint-modell er motsatsen, men Urds to + radnettet er et bevisst enkelhet-valg (D/besluttet); mellom innholdsbredden og mobil-breakpointet er lerretet flytende, innholdsdytten (ADR-0024) holder blokkene fra hverandre og hver blokk velger selv om innholdet brytes eller krymper (sidezoomen fra ADR-0018-tillegget 23. september 2026 ble bygd og trukket samme dag). Content push and shrink per block (0.7.17, 23 September 2026): a block whose content grows pushes the blocks below by the Wix Editor's gap rules (**C26**, ADR-0024), and any block can shrink its content instead of wrapping (`fit: "shrink"` with a floor, block-level); the proportional page `zoom` of the Tilda, Readymag and Wix Studio model was built and withdrawn the same day, since a page that only gets smaller adapts nothing.
...
| C19 | **Ribbons: a text marquee block and an image ribbon view** (items, separator, speed, direction, pause on hover; a static row under reduced motion) | 0.7.13.7 | ApeironLF's frieze and image marquee; a standard decorative element that needs no library |
| C20 | **Background motion, a photos layer, a pattern layer and shape dividers** (Ken Burns and drift on the image layer, scattered photos with drift or crossfade, inline SVG pattern tiles, wave/tilt/curve/triangle/zigzag dividers) | 0.7.13.8 (motion and photos) and 0.7.13.10 (pattern and dividers) | The four ApeironLF hero styles and the survey's cheap wins; Elementor and Divi have dividers, Divi and Wix patterns; WebGL art, Lottie and Spline are D (dependencies or GPU-only) |
| C21 | **Announcement bar and link launcher** (a strip above the nav that sticks or scrolls away; a grid of links behind a drawn button in the tool cluster) | 0.7.13.5 and 0.7.13.6 | Squarespace and Shopify have the bar; the launcher is ApeironLF's shortcut grid brought to the visitor nav |
| C22 | **Animated images and self-hosted video as content** (GIF, animated WebP and APNG kept as they are with size caps; the video block with a file source, poster, loop and muted) | 0.7.13.12 | Today every upload is re-encoded to webp and a video file exists only as a background layer |
| C23 | **Full-screen mobile menu and the transparent-to-solid nav** (a `<dialog>` sheet as a choice; `atTop: 'clear'` with the surface appearing on scroll) | 0.7.13.4 and 0.7.13.3 | ApeironLF's drawer and nav; Wix Studio's «Color while scrolling» |
| C24 | **Archiving and entry states** (one-action archive and restore, `archived` and `archivedAt`, kicker and urgent on news, section hide, a people kind with boards per period) | 0.7.16 (ADR-0025) | ApeironLF archives posters, news and boards instead of deleting them; a site owner without a developer needs it in one action |
| C25 | **Collection filters** (chips by year, period or category, tabs, a client-side search field) | 0.7.16.3 | ApeironLF's period chips, curriculum tabs and search; no library |
| C26 | **Content that grows must push, and blocks that shrink instead of wrap** (the Wix Editor's push rules on the absolute frames, ADR-0024; `fit: "shrink"` with a floor on any block; the proportional page `zoom` of the ADR-0018 addendum was withdrawn after testing) | Delivered 0.7.17.2 (the stages 0.7.17.2 and 0.7.17.3, 23 September 2026; the two surveys in LAERDOMMER §2; the test-round follow-up 0.7.17.4 is open) | Text wrapped over the blocks beneath at every width narrower than the design; Squarespace grows grid rows, Wix pushes by gap rules, the stack builders hug; Urd keeps its frames and pushes |

**Notater (ikke egne punkter):** komponent-/symbolgjenbruk: blokkgrupper som uavhengige kopier er levert (0.6.7.5), synkede symboler er fortsatt bare et notat; megameny og scroll-scrub kun ved dokumentert behov; planlagt publisering begrenses av statisk vert (publiser-ved-dato-filter er nærmeste); nettbrett-breakpoint forblir bevisst utsatt: mellom innholdsbredden og mobil-breakpointet er lerretet flytende, innholdsdytten holder blokkene fra hverandre og hver blokk kan krympe innholdet til et gulv (0.7.17, ADR-0024); sidezoomen ble bygd og trukket 23. september 2026.
