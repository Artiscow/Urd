...
Feltstudien støtter formen. Bransjekonsensus for 2026 er 1140 til 1200 px innhold med fullbredde bakgrunn. Framers standard skrivebords-brekkpunkt er 1200 px, og mobilen deres er 390 px, som er nøyaktig verdien `MOBILE_W` i Urds editor allerede bruker. Squarespaces vanlige standard er også 1200 px, der Fluid Engines tolv kolonner blir omtrent 100 px hver. Bootstraps xl-container er 1140 px. Wix Studio er unntaket med 1600 px standard, men lar bredden settes ned til 1001 px, og oppfører seg over maksbredden nøyaktig som modellen her: elementene beholder faste størrelser og siden får sidemarger i stedet for å fortsette å skalere.
...
3. **Bredden bor to steder, begge additive (ADR-0005).** `site.layout = { contentWidth, gutter }` er nettstedets designkonstant, med `contentWidth` som tall i px eller strengen `"full"`. Tall og ikke CSS-streng, fordi editoren må regne aritmetisk på verdien for forhåndsvisningens målbredde. `section.size.maxWidth` er per-seksjon overstyring og rømningsluke for fullbredde-helter og kant-til-kant skillelinjer. Feltet fantes allerede i `schema/page.schema.json`, ble aldri lest av motoren og var ikke dokumentert; det tas i bruk og dokumenteres i samme commit.

   **Standard er `contentWidth: 1200`, `gutter: 24`.** 1440 ble vurdert og forkastet: den binder først over 1488 px vindusbredde, altså nesten ingen bærbare, så vi ville fått nesten ingen invarians i praksis.

4. **Bindingen gjelder kun over mobilbrekkpunktet.** Under `body.urd-mobile` er margen 0 og kanvasen 100 %, så alle eksisterende `frames.mobile` beholder nøyaktig betydningen sin. Mobilmodellen er milepæl 0.7.3 og røres ikke her.

5. **Ingen grandfather: alt konverteres nå.** Ingen ordentlige sider er bygget med Urd ennå, så pre-v1-klausulen i ADR-0005-addendumet gjelder, og vi slipper å vedlikeholde to layoutmodeller for alltid. `SITE_SCHEMA_VERSION` bumpes, og migreringen fyller inn `layout` eksplisitt på filer som mangler det, slik at feltet alltid er til stede etter lasting og editor og motor aldri kan utlede ulik standard.

## Konsekvenser

- **En klon laget fra v0.6.11 med eget innhold vil reflowe ved oppdatering.** Det er den direkte prisen for punkt 5, og det sies her i klartekst i stedet for å oppdages. Fra v1.0 ville samme grep krevd en ekte migreringsvei med bevart utseende.

- **Grepet gir IKKE breddeinvarians generelt.** I båndet mellom mobilbrekkpunktet (640) og designbredden (1200) er `x`/`w` fortsatt prosent mens `y`/`h` er piksler, altså nøyaktig dagens forvrengning. Grepet AVGRENSER den til et endelig bånd på omtrent 560 px i stedet for et ubegrenset område. Ekte fiks er enten et nettbrett-brekkpunkt (0.7.3) eller Wix Studios modell der begge akser ganges med samme faktor, som forutsetter at typografien skalerer med. ADR-en skal ikke leses som en påstand om invarians under designbredden.
...
- **Forhåndsvisningen får en kanonisk mål-viewport, og 0.6.6.5.9 sitt fyll-valg blir et modusvalg.** Fold-avviket har to uavhengige årsaker: at layouten avhenger av admin-vinduets bredde (som denne ADR-en fjerner ved å pinne målbredden), og at lerretet har annet sideforhold enn en ekte nettleser, slik at `100vh` løses mot lerretet (som denne ADR-en ikke rører). Den andre halvparten løses ved å tilpasse på begge akser mot en fast mål-viewport. Prisen er at aksen med overskudd får en bar, altså en delvis reversering av 0.6.6.5.9 sitt bevisste valg om ingen topp- og bunnbarer. Fyll beholdes derfor som et eksplisitt modusvalg ved siden av dagens Tilpass- og 100 %-kontroll i stedet for å bli slettet.

  Feltstudien var avgjørende her: Webflow gjør i dag det Urd gjør, og brukerne rapporterer nøyaktig vårt symptom («very inaccurate and misleading when working with background-image, percentage heights and vh units»). Både høydekontroll og en above-the-fold-linje har ligget som ubygde ønsker der i årevis, så foldlinje-varianten er ikke validert av noen. Squarespace løser det med enhetsramme som endrer selve sideframen, og det er modellen vi tar.
