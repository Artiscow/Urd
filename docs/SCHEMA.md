...
- **The `calendar` block's props** (core from 0.7.18.4; the same shape as the former calendar plugin, and the legacy id `kalender` resolves to it): `{ "sources": [], "view": "list" | "cards" | "month" | "next", "limit": 6, "showCategories": true, "showSubscribe": true }`. A source is an iCal URL, a `webcal://` URL or a Google calendar id; feeds are fetched through the site's own `/api/ics` proxy (host allowlist: Google plus `ICS_HOSTS`), parsed by the pure `ics.js` (loaded on the first render, outside the visitor closure), and cached for ten minutes in sessionStorage. «Category: Title» gives category chips, a «Sign-up: …» line in the description gives a button. The sources, the view, the count and the two toggles are edited in the Properties panel (Content). The `whats-on` section preset (legacy id `hva-skjer`) is a title and a calendar.
- **The `form` block's props** (core from 0.7.18.3; the same shape as the former form plugin, and the legacy id `skjema` resolves to it): `{ "recipient": "", "subject": "", "mode": "mailto" | "endpoint", "endpoint": "", "submitLabel": "Send", "successText": "…", "fields": [{ "id", "label", "type", "required", "options"? }] }` with the field types text, email, tel, textarea, select, checkbox, radio and date (`options` only for select and radio). Validation, the honeypot, the mailto URL and the endpoint payload are the pure `form-model.js`; endpoint mode needs connect-src for the endpoint host in `_headers`. The send mode, recipient or endpoint, the field list and the texts are edited in the Properties panel (Content). The `contact-form` section preset (legacy id `kontaktskjema`) is an intro text and a form.
...
- Without `fields`, Properties shows as before a «Settings …» button that opens the plugin's own config panel in the preview (`urd-open-block-config`). The core map block uses the same field types through the editor's `CORE_FIELDS` table; the core calendar and form blocks have their own Content panels in the editor.
