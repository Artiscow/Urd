...
- [ ] Engangsoppsettet (rekkefølgen i UTVIKLING): opprett offentlig `urd-template`-repo MED «Add a README file» (så main finnes), legg secreten `URD_TEMPLATE_PAT` (fine-grained, contents read/write kun på malrepoet) i monorepoet, kjør første synk, og huk deretter av Settings → General → «Template repository» (da vises «Use this template»-knappen) + sett topic `urd-mal`
