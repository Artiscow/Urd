...
- **Nye feilkoder i functions krever `api.<kode>`-nøkler i alle fem admin-ordbøker** (kontraktstesten i tests/i18n.test.mjs håndhever). Functions har aldri locale-infrastruktur: norsk `error`-tekst er fallback, klienten oversetter med `taApiError` og sammenligner ALDRI mot norske literaler.
- **Nye UI-nøkler går i alle fem ordbøkene med {var}-paritet** (paritetstesten håndhever); admin-ordbøkene buntes aldri, så oversettelser krever ikke gjenbygg. `ta()` kalles aldri på modulnivå, og seed-tekst (brukerdata) oversettes ÉN gang ved innsetting, aldri ved rendring (ADR-0012).
